...
- Учтена возможность добавления новых платёжных систем путём добавления реализаций `PaymentProcessorInterface` в `src/Service/PaymentProcessor`.
- Для наполнения базы данных тестовыми данными используются фикстуры.
- Код покрыт PHPUnit тестами с покрытием 100%.

## Команды для работы с окружением

Следующие команды выполняются в контейнере `sio_test`. Для входа в контейнер выполните:
...
Для выполнения миграций и загрузки фикстур используйте следующие команды:
...
Для запуска тестов выполните одну из команд:

- Для вывода покрытия в текстовом формате:

```bash
XDEBUG_MODE=coverage APP_ENV=test php bin/phpunit --coverage-text
```

- Для генерации отчёта покрытия в HTML:

```bash
XDEBUG_MODE=coverage APP_ENV=test php bin/phpunit --coverage-html var/coverage
```
